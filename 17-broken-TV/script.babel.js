...
function intRandom(start, end) {
  if (Object.prototype.toString.call(start) !== '[object Number]') throw new TypeError("".concat(start, " is not a number"));
  if (Object.prototype.toString.call(end) !== '[object Number]') throw new TypeError("".concat(end, " is not a number"));
...
function createCutLine() {
  ...
  cutLine.sort(function (a, b) {
    return a - b;
  });
}

function drawNoise() {
  var imageData = cvsCtx.createImageData(w, h);
...
function drawText() {
  ...
  cvsCtx.font = "bold ".concat(fontSize, "px Helvetica");
  cvsCtx.textAlign = 'center';
  cvsCtx.textBaseline = 'middle';
  createCutLine();

  for (var i = 0; i < cutLine.length - 1; i++) {
    cvsCtx.save();
    cvsCtx.beginPath();
...
function drawStripe() {
  cvsCtx.beginPath();
  var f = cvsCtx.createLinearGradient(0, -stripeOffset, 0, 2 * h - stripeOffset);
