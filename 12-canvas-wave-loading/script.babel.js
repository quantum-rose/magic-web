...
var $canvas = document.getElementById('wave'),
    ...
    PI_1_2 = Math.PI / 2,
    PI_2 = Math.PI * 2;
var cvsSize = $canvas.offsetWidth * window.devicePixelRatio,
...
CanvasRenderingContext2D.prototype.roundedRect_arc = function (x, y, width, height, radius) {
  var inset = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : false;
  var per = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : 1;
  inset = !!inset;
  per = between01(isNaN(per) ? 1 : per);
  var l = width * 2 + height * 2 - radius * 8 + Math.PI * radius * 2;
  var w = width - radius * 2;
  var pw = w / l;
  var h = height - radius * 2;
  var ph = h / l;
  var arc = Math.PI * radius * 2 / 4;
  var parc = arc / l;
  cvsCtx.moveTo(x + radius, y);
  ...
  if (inset) {
    ...
    cvsCtx.arc(x + width - radius, y + radius, radius, Math.PI / -2, Math.PI / -2 * (1 - between01((per - pw) / parc)), false);
  }
  ...
  if (inset) {
    cvsCtx.arc(x + width, y + height, radius, Math.PI / -2, Math.PI / -2 * (1 + between01((per - pw - parc - ph) / parc)), true);
  } else {
    cvsCtx.arc(x + width - radius, y + height - radius, radius, 0, Math.PI / 2 * between01((per - pw - parc - ph) / parc), false);
  }
  ...
  if (inset) {
    ...
    cvsCtx.arc(x + radius, y + height - radius, radius, Math.PI / 2, Math.PI / 2 * (1 + between01((per - 2 * (pw + parc) - ph) / parc)), false);
  }
  ...
  if (inset) {
    cvsCtx.arc(x, y, radius, Math.PI / 2, Math.PI / 2 * (1 - between01((per - 2 * (pw + ph) - 3 * parc) / parc)), true);
  } else {
    cvsCtx.arc(x + radius, y + radius, radius, Math.PI, Math.PI / 2 * (2 + between01((per - 2 * (pw + ph) - 3 * parc) / parc)), false);
  }
};
...
function drawRoundedRect(x, y, width, height, radius, per) {
  ...
  if (isDynamicInsetRoundedRect) {
    cvsCtx.beginPath();
    cvsCtx.roundedRect_arc(outlineWidth / 4, outlineWidth / 4, cvsSize - outlineWidth / 2, cvsSize - outlineWidth / 2, radius * 2 - outlineWidth / 4, true, per);
...
function draw() {
  if (Date.now() - lastTime > 16) {
    lastTime = Date.now();
    cvsCtx.clearRect(0, 0, cvsSize, cvsSize); // percent = 0.5;
...
function drawWave(per) {
  ...
  cvsCtx.beginPath();
  cvsCtx.moveTo(outlineWidth, Math.pow(p, 0.5) * A * Math.sin(fi) + p * clipCircleDiameter + outlineWidth);

  for (var i = 1; i <= clipCircleDiameter; i++) {
    cvsCtx.lineTo(i + outlineWidth, Math.pow(p, 0.5) * A * Math.sin(ω * i + fi) + p * clipCircleDiameter + outlineWidth);
  }

  cvsCtx.lineTo(waveBottomRight, waveBottomRight);
  cvsCtx.lineTo(outlineWidth, waveBottomRight);
  cvsCtx.closePath();
  var fill = cvsCtx.createLinearGradient(outlineWidth, outlineWidth, outlineWidth, waveBottomRight);
  fill.addColorStop(0, 'rgba(10, 12, 32, 0.5)');
  fill.addColorStop(0.5, "rgba(".concat(44 - per * 34, ", ").concat(75 - per * 63, ", ").concat(80 - per * 48, ", 0.5)"));
  fill.addColorStop(1, "rgba(".concat(77 - per * 67, ", ").concat(146 - per * 134, ", ").concat(134 - per * 102, ", 0.5)"));
